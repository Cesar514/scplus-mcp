...
func Discover() (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("scplus-mcp backend entrypoint %q is missing: %w", entry, err)
	}
	if info.IsDir() {
		return nil, fmt.Errorf("scplus-mcp backend entrypoint %q is a directory", entry)
	}
	client := &Client{
		nodeBin: nodeBin,
		entry:   entry,

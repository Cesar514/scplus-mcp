# scplus MCP - Agent Instructions

## Purpose

You are equipped with the scplus MCP server. It gives you structural awareness of the entire codebase without reading every file. Follow this workflow strictly to conserve context and maximize accuracy.
...
- `static-analysis.ts` - Native linter runner (tsc, eslint, py_compile, cargo check, go vet) plus repo/file scoring from scplus rule findings.
...
| Tool                         | When to Use                                                                        |
| ---------------------------- | ---------------------------------------------------------------------------------- |
| `index`                      | Build or refresh repo-local `.scplus/` state. Full mode is the default and persists the authoritative sqlite index substrate at `.scplus/state/index.sqlite`, including file, identifier, chunk, structure, restore-point, and context-tree state with no JSON mirrors. |

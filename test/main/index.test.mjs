...
function readArtifactFromDb(dbPath, artifactKey) {
  ...
  try {
    ...
    const row = db.prepare("SELECT artifact_json FROM index_artifacts WHERE artifact_key = ?").get(qualifyArtifactKey(artifactKey, generation));
    if (!row) return null;
...
function readTextArtifactFromDb(dbPath, artifactKey) {
  ...
  try {
    const generation = getActiveGenerationFromDb(dbPath);
    const row = db.prepare("SELECT artifact_text FROM index_text_artifacts WHERE artifact_key = ?").get(qualifyArtifactKey(artifactKey, generation));
...
function readMetaFromDb(dbPath, metaKey) {
  const db = new DatabaseSync(dbPath);
  try {
    const row = db.prepare("SELECT meta_value FROM index_db_meta WHERE meta_key = ?").get(metaKey);
...
function qualifyArtifactKey(artifactKey, generation) {
  ...
  return `generation:${generation}:${artifactKey}`;
}
...
describe("index", () => {
  it("creates the populated .scplus project layout and snapshots", async () => {
    ...
    try {
      ...
      const queryExplanationIndex = readArtifactFromDb(dbPath, "query-explanation-index");
      ...
      assert.equal(config.artifactVersion, 17);
      ...
      assert.equal(manifest.contractVersion, 13);
      assert.equal(manifest.indexMode, "full");
      assert.equal(indexStatus.state, "completed");
      assert.equal(indexStatus.phase, "completed");
      assert.equal(indexStatus.runGeneration, 1);
      assert.equal(indexStatus.activeGeneration, 1);
      assert.equal(indexStatus.pendingGeneration, null);
      assert.equal(indexStatus.latestGeneration, 1);
      assert.equal(indexStatus.activeGenerationFreshness, "fresh");
      assert.equal(indexStatus.indexMode, "full");
      assert.equal(indexStatus.contractVersion, 13);
      assert.equal(indexStatus.artifactVersion, 17);
      ...
      assert.ok(indexStatus.stageOrder.includes("explanation-scan"));
      assert.equal(stageState.generation, 1);
      assert.equal(stageState.mode, "full");
      assert.equal(stageState.contractVersion, 13);
      ...
      assert.equal(indexStatus.observability.stages["file-search"].durationMs > 0, true);
      assert.equal(indexStatus.observability.stages["identifier-search"].durationMs > 0, true);
      assert.equal(indexStatus.observability.stages["full-artifacts"].durationMs > 0, true);
      assert.equal(typeof indexStatus.observability.stages["file-search"].filesPerSecond, "number");
      assert.equal(typeof indexStatus.observability.stages["full-artifacts"].chunksPerSecond, "number");
      assert.ok(fileIndex.files["src/app.ts"]);
      assert.equal(identifierIndex.files["src/app.ts"].docs.some((doc) => doc.name === "run"), true);
      assert.equal(chunkIndex.artifactVersion, 17);
      assert.equal(chunkIndex.contractVersion, 13);
      ...
      assert.equal(hybridChunkIndex.artifactVersion, 17);
      assert.equal(hybridChunkIndex.contractVersion, 13);
      ...
      assert.equal(hybridIdentifierIndex.artifactVersion, 17);
      assert.equal(hybridIdentifierIndex.contractVersion, 13);
      ...
      assert.equal(structureIndex.artifactVersion, 17);
      assert.equal(structureIndex.contractVersion, 13);
      ...
      assert.equal(semanticClusterIndex.artifactVersion, 17);
      assert.equal(semanticClusterIndex.contractVersion, 13);
      ...
      assert.equal(hubSuggestionIndex.artifactVersion, 17);
      assert.equal(hubSuggestionIndex.contractVersion, 13);
      ...
      assert.equal(queryExplanationIndex.artifactVersion, 17);
      assert.equal(queryExplanationIndex.contractVersion, 13);
      assert.equal(queryExplanationIndex.mode, "full");
      assert.equal(typeof queryExplanationIndex.queryEngine.exact.label, "string");
      assert.equal(typeof queryExplanationIndex.fileCards["src/app.ts"].purposeSummary, "string");
      assert.equal(typeof queryExplanationIndex.fileCards["src/app.ts"].publicApiCard, "string");
      assert.equal(typeof queryExplanationIndex.moduleCards["src"].purposeSummary, "string");
      assert.equal(fullManifest.mode, "full");
      assert.equal(fullManifest.generation, 1);
      assert.equal(fullManifest.artifactVersion, 17);
      assert.equal(fullManifest.contractVersion, 13);
      ...
      assert.equal(fullManifest.queryExplanationCount >= 1, true);
      ...
      assert.equal(dbFullManifest?.queryExplanationIndexPath, "sqlite:index_artifacts/query-explanation-index");

...
describe("init-codex", () => {
  it("writes valid .codex/config.toml for npx runner", async () => {
    ...
    try {
      ...
      assert.match(raw, /^\[mcp_servers\.contextplus\]$/m);
      assert.match(raw, /^command = "npx"$/m);
      assert.match(raw, /^args = \["-y","contextplus"\]$/m);
      assert.match(raw, /^\[mcp_servers\.contextplus\.env\]$/m);
  ...
  it("writes valid .codex/config.toml for bunx runner", async () => {
    ...
    try {
      ...
      assert.match(raw, /^args = \["contextplus"\]$/m);
    } finally {
      await rm(cwd, { recursive: true, force: true });
    }
  });
